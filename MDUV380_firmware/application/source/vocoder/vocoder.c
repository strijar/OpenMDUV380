...
#include "vocoder.h"
#include "vocoder_asm.h"
#include "portable_initial_state.h"

const unsigned char BIT_MASK_TABLE[] = { 0x80U, 0x40U, 0x20U, 0x10U, 0x08U, 0x04U, 0x02U, 0x01U };

#define WRITE_BIT(p,i,b) p[(i)>>3] = (uint8_t)((b) ? \
    (p[(i)>>3] | BIT_MASK_TABLE[(i)&7]) : \
    (p[(i)>>3] & (uint8_t)~BIT_MASK_TABLE[(i)&7]))
...
_Alignas(uint64_t) static uint8_t decoder_state[VOCODER_DECODER_STATE_SIZE];
_Alignas(uint64_t) static uint8_t encoder_state[VOCODER_ENCODER_STATE_SIZE];
static int state_initialized;

static void ensure_state_initialized(void)
{
    if (state_initialized == 0) {
        memcpy(decoder_state, vocoder_decoder_initial_state,
               sizeof(decoder_state));
        memcpy(encoder_state, vocoder_encoder_initial_state,
...
{
	memcpy(decoder_state, vocoder_decoder_initial_state, sizeof(decoder_state));
	memcpy(encoder_state, vocoder_encoder_initial_state, sizeof(encoder_state));
	state_initialized = 1;
	return 0;
}

void vocoder_encode(uint8_t *vocoder49, const int16_t *pcm)
{
	int16_t vocoder[49] = {0};
	ensure_state_initialized();
	
	memset(vocoder49, 0, 7);
	(void)asm_encode_thing(
		vocoder, 0, pcm, 0x50, 0x1840U, 0, 0x2000, encoder_state);
	(void)asm_encode_thing(
		vocoder, 0, pcm + 80, 0x50, 0x1840U, 1, 0x2000, encoder_state);

	for(int i = 0; i < 6; ++i){
		for(int j = 0; j < 8; ++j){
			vocoder49[i] = (uint8_t)(vocoder49[i] |
				(uint8_t)((uint16_t)vocoder[(i * 8) + (7 - j)] << j));
		}
	}
    vocoder49[6] = vocoder[48] ? 0x80 : 0;
}
...
void vocoder_encode_fec(uint8_t *vocoder, const int16_t *pcm)
{
	...
	uint8_t vocoder49[9] = {0};

	vocoder_encode(vocoder49, pcm);
	
	for (unsigned int i = 0U; i < 12U; i++, MASK >>= 1) {
		unsigned int n1 = i + 0U;
		unsigned int n2 = i  + 12U;
		if (READ_BIT(vocoder49, n1))
			aOrig |= MASK;
		if (READ_BIT(vocoder49, n2))
			bOrig |= MASK;
	}
	...
	for (unsigned int i = 0U; i < 24U; i++, MASK >>= 1) {
		...
		WRITE_BIT(vocoder, aPos, a & MASK);
	}

	MASK = 0x400000U;
	for (unsigned int i = 0U; i < 23U; i++, MASK >>= 1) {
		unsigned int bPos = B_TABLE[i];
		WRITE_BIT(vocoder, bPos, b & MASK);
	}

	MASK = 0x1000000U;
	for (unsigned int i = 0U; i < 25U; i++, MASK >>= 1) {
		unsigned int cPos = C_TABLE[i];
		WRITE_BIT(vocoder, cPos, cOrig & MASK);
	}
}

void vocoder_decode(const uint8_t *vocoder49, int16_t *pcm)
{
	int16_t vocoder[49];
	ensure_state_initialized();
    ...
    for(int i=0;i<6;i++){
      for(int j=0;j<8;j++){
        vocoder[vocoderi++] = (int16_t)(
            ((uint32_t)vocoder49[i] >> (unsigned int)(7 - j)) & 1U);
      }
    }
    vocoder[vocoderi++]=vocoder49[6] ? 1 : 0;

    (void)asm_decode_wav(
        pcm, 80, vocoder, 0, 0, 0, decoder_state);
    (void)asm_decode_wav(
        pcm + 80, 80, vocoder, 0, 0, 1, decoder_state);
}
...
void vocoder_decode_fec(const uint8_t *vocoder, int16_t *pcm)
{
	...
	for (unsigned int i = 0U; i < 12U; i++, MASK >>= 1) {
		...
		WRITE_BIT(vocoder49, aPos, a & MASK);
		WRITE_BIT(vocoder49, bPos, b & MASK);
	}

	MASK = 0x1000000U;
	for (unsigned int i = 0U; i < 25U; i++, MASK >>= 1) {
		unsigned int cPos = i + 24U;
		WRITE_BIT(vocoder49, cPos, c & MASK);
	}

	vocoder_decode(vocoder49, pcm);
}
